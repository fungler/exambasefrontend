...
const Content = props => {
  return (
    <Switch>
      <Route path="/home">
        <Home dataFromServer={props.dataFromServer} />
      </Route>
      <Route path="/people">
        <People
          props={(props.listPeople, props.setListPeople, props.setListPeople)}
        />
      </Route>
      <Route path="/company">
        <Company />
...
function Home(props) {
  useEffect(() => {
    facade.fetchPeople().then(res => props.setListPeople(res));
  }, []);
  useEffect(() => {
    facade.fetchData().then(res => props.setDataFromServer(res.msg));
  }, []);
